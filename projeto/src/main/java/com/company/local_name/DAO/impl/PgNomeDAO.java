...
public class PgNomeDAO implements NomeDAO {
    ...
    private static final String UPDATE_NOME   = "UPDATE local_names_db.nome SET frequencia=?, rank=? WHERE nome = ? AND localidade = ?";
    ...
    public Nome get(Object key) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(GET_NOME)) {
            ...
            while (statement.getResultSet().next()) {
                nome = new Nome(
                    statement.getResultSet().getString("localidade"),
                    statement.getResultSet().getString("nome"),
                    statement.getResultSet().getInt("frequencia"),
                    statement.getResultSet().getInt("rank")
                );
            }
        } catch (SQLException ex) {
            System.err.println(ex.getMessage());
    ...
    public List<Nome> getAll() throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(GET_ALL_NOMES)) {
            ...
            System.err.println(ex.getMessage());
            throw new SQLException("Erro ao listar regiões");
        }
    ...
    public void update(Nome object) throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement(UPDATE_NOME)) {
            statement.setInt(1, object.getFrequencia());
            statement.setInt(2, object.getRank());
            statement.setString(3, object.getNome());
            statement.setString(4, object.getCidade());
